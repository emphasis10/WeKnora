<template>
  <div class="kb-list-container">
    ...
    <!-- Uninitialized Knowledge Base Tip -->
...
interface KB { 
  ...
  chunk_count?: number;
  isProcessing?: boolean; // Whether there are import tasks being processed
...
onMounted(() => {
  fetchList().then(() => {
    // Check if there is a knowledge base ID that needs to be highlighted in route parameters
...
// Listen to route changes, handle highlight requests from other pages
...
const openMore = (index: number) => {
  // Only record current opened index for active style
  // Modal open/close is automatically managed by v-model
...
// Check if there are uninitialized knowledge bases
...
const handleKBEditorSuccess = (kbId: string) => {
  ...
  fetchList().then(() => {
    // If highlight KB ID is from route params, trigger flash effect
