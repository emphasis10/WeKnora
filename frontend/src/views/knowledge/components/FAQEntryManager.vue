      ...
      <div class="faq-editor-drawer-content">
        ...
        >
          <div class="settings-group">
            ...
            <!-- Negative Questions (Negative samples) -->
...
// Scroll loading more for tag list
...
const handleFaqMenuAction = (event: Event) => {
  ...
  if (detail.action === 'create') {
    ...
    // Batch operations handled via the dropdown menu on the left sidebar
...
const handleEntryStatusChange = async (entry: FAQEntry, value: boolean) => {
  ...
  // Get actual object reference from array to ensure latest data is used
  const actualEntry = entries.value[entryIndex]
  const previous = actualEntry.is_enabled
  if (previous === value) {
    return
  }
  // Update property directly, Vue 3's reactivity system should detect it
...
const loadEntries = async (append = false) => {
  ...
  try {
    ...
    const newEntries = (pageData.data || []).map(entry => ({
      ...
      negativeCollapsed: true,  // Negative questions collapsed by default
    ...
    // Wait for DOM update and re-layout
    await nextTick()
    arrangeCards()
  } catch (error: any) {
    MessagePlugin.error(error?.message || t('common.operationFailed'))
    ...
    // Check if continued loading is needed to fill the visible area
...
const handleScroll = () => {
  ...
  // Load more when scrolled to 200px from bottom
  if (scrollTop + clientHeight >= scrollHeight - 200) {
    loadEntries(true)
  }
}

// Check if content fills the visible area; if not and more data exists, continue loading
const checkAndLoadMore = () => {
  ...
  // If content height is less than container height + 50px buffer, it might mean no scrollbar or near bottom, so load more
...
const openImportDialog = () => {
  // If importing is in progress, do not allow opening the import dialog
  if (importState.taskStatus?.status === 'running') {
    MessagePlugin.warning('Import is in progress, please wait until it completes.')
    return
  }
  stopPolling()
  ...
  // Note: Do not clear taskId and taskStatus so progress can still be seen after closing the dialog
...
const parseCSVFile = async (file: File): Promise<FAQEntryPayload[]> => {
  ...
  // Use papaparse to parse CSV, automatically handling quotes, escapes, delimiters, etc.
  return new Promise((resolve, reject) => {
    Papa.parse(text, {
      header: true,
      skipEmptyLines: true,
      delimiter: '', // Automatically detect delimiters (comma or tab)
      quoteChar: '"',
      escapeChar: '"',
      transformHeader: (header: string) => {
        // Remove parentheses and descriptions from field names, keeping only core field names
          ...
          results.data.forEach((row: any) => {
            ...
            payloads.push(
              normalizePayload({
                ...
                tag_name: record['分类'] || record['Category'] || record['tag_name'] || '',
                is_enabled: isDisabled !== undefined ? !isDisabled : undefined, // Is Disabled: FALSE means enabled, TRUE means disabled, so negate
              }),
            )
          })
          resolve(payloads)
...
const parseExcelFile = async (file: File): Promise<FAQEntryPayload[]> => {
  ...
  // Use raw: false to ensure proper handling of quotes and escapes
  const json = XLSX.utils.sheet_to_json<Record<string, string>>(worksheet, { 
    defval: '',
    raw: false // Ensure string values are correctly parsed
  })
  return json.map((row) => {
    // Get raw headers (removing parentheses descriptions)
    ...
    const isDisabled = parseBooleanField(normalizedRow['Disabled'] || normalizedRow['是否停用'], false)
    return normalizePayload({
      standard_question: normalizedRow['Question'] || normalizedRow['问题'] || normalizedRow['standard_question'] || normalizedRow['question'] || '',
      answers: splitByDelimiter(normalizedRow['Answers'] || normalizedRow['机器人回答'] || normalizedRow['answers']),
      similar_questions: splitByDelimiter(normalizedRow['Similar Questions'] || normalizedRow['相似问题'] || normalizedRow['similar_questions']),
      negative_questions: splitByDelimiter(normalizedRow['Negative Questions'] || normalizedRow['反例问题'] || normalizedRow['negative_questions']),
      tag_id: normalizedRow['tag_id'] || '',
      tag_name: normalizedRow['Category'] || normalizedRow['分类'] || normalizedRow['tag_name'] || '',
      is_enabled: isDisabled !== undefined ? !isDisabled : undefined, // Is Disabled: FALSE means enabled, TRUE means disabled, so negate
    })
  })
}
...
const splitByDelimiter = (value?: string) => {
  ...
  // If ## delimiter is present, split by ##
  ...
  // If no ## delimiter, the whole value is treated as one answer
  return [trimmedValue]
}

// Parse boolean fields (supports multiple formats: TRUE/FALSE, true/false, Yes/No, 1/0, etc.)
const parseBooleanField = (value?: string, defaultValue: boolean = true): boolean | undefined => {
  if (!value) return undefined
  const normalized = value.trim().toUpperCase()
  if (normalized === 'TRUE' || normalized === '1' || normalized === 'Yes' || normalized === 'YES' || normalized === '是') {
    return true
  }
  if (normalized === 'FALSE' || normalized === '0' || normalized === 'No' || normalized === 'NO' || normalized === '否') {
    return false
  }
  return defaultValue
}
  ...
  importState.pollingInterval = setInterval(async () => {
    try {
      ...
      if (progressData) {
        ...
        // Refresh FAQ list when progress updates (refreshes after every few items)
        ...
        // Task completed or failed, stop polling (but don't automatically close progress bar, let user close it manually)
        if (status === 'success' || status === 'failed') {
          stopPolling()
          if (status === 'success') {
            MessagePlugin.success(t('knowledgeEditor.faqImport.importSuccess'))
            // Clear filter criteria to ensure user sees all newly imported data
            selectedTagId.value = UNTAGGED_FILTER
            entrySearchKeyword.value = ''
            overallFAQTotal.value = 0  // Reset to trigger re-fetch
            await loadEntries()
            await loadTags()
            // Task completed, automatically close progress bar after 3 seconds
            setTimeout(() => {
            ...
            // Don't auto-close on failure, let user see error message
          }
        }
      }
    } catch (error: any) {
...
const handleCancelImport = () => {
  ...
  // Note: Do not clear localStorage, as task might still be in progress
}
...
// Restore import task status (for recovery after refresh)
...
const handleImport = async () => {
  ...
  // If task completed or failed, close dialog
  ...
  try {
    ...
    if (taskId) {
      ...
      startPolling(taskId)
      // Close import dialog immediately, progress will be shown at the top of the list page
      importVisible.value = false
      // Reset import dialog state (keep taskId and taskStatus for progress display)
      importState.file = null
      importState.preview = []
      importState.importing = false
    } else {
      // If no task ID returned, it might be an old version of the API, use synchronous mode
      MessagePlugin.success(t('knowledgeEditor.faqImport.importSuccess'))
      importVisible.value = false
      await loadEntries()
      importState.importing = false
...
// Listen for selection changes and notify the left menu
watch(selectedRowKeys, (newKeys, oldKeys) => {
  const count = newKeys.length
  // Get status info of selected items
...
const exampleData: FAQEntryPayload[] = [
  {
    ...
    similar_questions: ['What\'s WeKnora?', 'Introduce WeKnora'],
  ...
  },
  {
    ...
    answers: ['Click the "New Knowledge Base" button', 'Select a knowledge base type and fill in the relevant information', 'Start using it after creation'],
    similar_questions: ['How do I create a KB?', 'How to add a knowledge base?'],
    negative_questions: [],
    tag_name: 'Usage Guide',
  },
]
...
const downloadCSVExample = () => {
  const headers = ['Category(Required)', 'Question(Required)', 'Similar Questions(Optional - Split by ##)', 'Negative Questions(Optional - Split by ##)', 'Bot Answers(Required - Split by ##)', 'Reply All(Optional - Default FALSE)', 'Disabled(Optional - Default FALSE)', 'No Recommendation(Optional - Default False)']
  ...
  const csvContent = [
    ...
    ...rows.map((row) => row.map((cell) => {
      // If contains tab, newline, or quotes, wrap with quotes
...
const downloadExcelExample = () => {
  const worksheet = XLSX.utils.json_to_sheet(
    exampleData.map((item) => ({
      ...
      'Similar Questions(Optional - Split by ##)': item.similar_questions.join('##'),
      'Negative Questions(Optional - Split by ##)': item.negative_questions.join('##'),
      'Bot Answers(Required - Split by ##)': item.answers.join('##'),
      'Reply All(Optional - Default FALSE)': 'FALSE',
      'Disabled(Optional - Default FALSE)': 'FALSE',
      'No Recommendation(Optional - Default False)': 'FALSE',
    })),
  )
...
watch(
  ...
  async (newKbId) => {
    ...
    if (!newKbId) {
      kbInfo.value = null
      // Clear previous task status when kbId changes
      stopPolling()
      importState.taskId = null
      importState.taskStatus = null
      clearTaskIdFromStorage()
...
// Listen for FAQ search keyword changes
...
const arrangeCards = () => {
  ...
  requestAnimationFrame(() => {
    // Set width first, keep current position unchanged
    cards.forEach((card) => {
    ...
    requestAnimationFrame(() => {
      // Calculate height of all cards (without changing position)
      ...
      // Update all card positions in batch, using CSS transitions for smooth movement
      cards.forEach((card, index) => {
        ...
        // If position changed, add transition effect
      ...
      // Remove transition and will-change after animation to avoid affecting future interactions
      setTimeout(() => {
        cards.forEach((card) => {
...
// Listen for window resize (using debounce)
...
const handleResize = () => {
  ...
  resizeTimer = setTimeout(() => {
    arrangeCards()
    // May need to load more when window expands, delay to ensure layout completion
    setTimeout(() => {
      checkAndLoadMore()
    }, 350)
    resizeTimer = null
...
onMounted(async () => {
  ...
  // Restore import task status if kbId exists
  if (props.kbId) {
    await restoreImportTask()
  }
  // Trigger selection count event once to ensure left menu receives initial state
  nextTick(() => {
...
// Listen for entries changes, re-layout
...
// Listen for collapse state changes, re-layout (using debounce and animation completion callback)
watch(() => entries.value.map(e => ({
  ...
  nextTick(() => {
    // Wait for a render frame to let height changes take effect
    requestAnimationFrame(() => {
      // Wait another render frame to ensure accurate height calculation
      requestAnimationFrame(() => {
        // Wait for Transition animation to complete before layout (slide-down duration ~200ms)
...
// Import progress bar style (displayed at the top of the list page)
...
// Card list style - use absolute positioning for waterfall layout, next row fills gaps in previous row
.faq-card-list {
  position: relative;
  width: 100%;
  min-width: 0;
...
.faq-card {
  ...
  // Note: top and left transitions are controlled by JS to avoid conflicts with layout animations
...
.faq-tags {
  ...
  // Limit max-width when there is only one tag in a line
...
.question-tag {
  ...
  // For span elements inside TDesign tag
  :deep(.t-tag span),
  :deep(.t-tag > span) {
...
// Import dialog style - consistent with knowledge base creation popup style
.faq-import-overlay {
  position: fixed;
  inset: 0;
  z-index: 1000;
...
// Radio button group style - consistent with project theme style
...
// Responsive layout calculated dynamically by JS, no media queries needed here

// Card menu popup style
:deep(.faq-card-popup) {
  z-index: 99 !important;
...
// FAQ Editor form style - fully refer to settings page
.faq-editor-form {
  width: 100%;

  // Hide default Form structure
  :deep(.t-form__label) {
...
.setting-row {
  ...
  // Emphasis style for main fields (standard question, answers)
  &.setting-row-primary {
    padding: 20px 0;
    padding-left: 12px;
    position: relative;

    // Remove top margin for the first one (standard question)
    &:first-child {
      padding-top: 0;
    }

    // Left color indicator (both standard question and answers use green)
    &::before {
      content: '';
      position: absolute;
      left: 0;
  ...
  // Secondary style for optional fields (similar questions, negatives)
  ...
  // Blue indicator for similar questions
  &.setting-row-similar::before {
    background: #3B82F6;
  }

  // Orange indicator for negatives
  &.setting-row-negative::before {
    background: #F59E0B;
  }

  // Remove bottom border for answers
  &.setting-row-answer {
    border-bottom: none;
  }
}
...
// Ensure full width for inputs in vertical layout
...
// Input component style - consistent with login page
:deep(.t-input) {
  font-family: "PingFang SC";
  font-size: 14px;
  border: 1px solid #E7E7E7;
...
// Batch tag modal style - consistent with import dialog style
.batch-tag-overlay {
  position: fixed;
  inset: 0;
  z-index: 1000;
