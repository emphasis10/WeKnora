...
func (t *DatabaseQueryTool) formatQueryResults(
	...
	output += fmt.Sprintf("Returned %d rows of data\n\n", len(results))
	...
	if len(results) > 10 {
		output += fmt.Sprintf("Note: Displaying the first %d records out of %d total. It is recommended to use the LIMIT clause to restrict the result set.\n", len(results), len(results))
	}
