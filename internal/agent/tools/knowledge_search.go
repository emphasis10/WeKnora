...
func (t *KnowledgeSearchTool) formatOutput(
	...
	for i, result := range results {
		...
		// Parse and output related image info
		if result.ImageInfo != "" {
		...
		// Add image info to structured data
		if result.ImageInfo != "" {
			var imageInfos []types.ImageInfo
			if err := json.Unmarshal([]byte(result.ImageInfo), &imageInfos); err == nil && len(imageInfos) > 0 {
				// Build simplified image info list
	...
	output += "\n=== Search Statistics & Recommendations ===\n\n"
...
// getEnrichedPassage merges content and image info text content
func (t *KnowledgeSearchTool) getEnrichedPassage(ctx context.Context, result *types.SearchResult) string {
	...
	// Extract captions and OCR text from all images
	...
	for _, img := range imageInfos {
		...
		if img.OCRText != "" {
			imageTexts = append(imageTexts, fmt.Sprintf("Image OCR Text: %s", img.OCRText))
		}
	}
